...
function SingleService() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        axiosInstance.get(`/services/${id}`).then((result) => {
          const data = result.data;

          setService(data);
        });
      } catch (error) {
        ToastAlert2.fire("Error", "Vui lòng thử lại" + error.message, "error");
      } finally {
        setLoading(false);
      }
    };
                ...
                <div className="group flex items-center gap-x-3 border-b border-gray-200 pb-8 mb-8 dark:border-neutral-700">
                  <a className="block shrink-0 focus:outline-none" href="#">
                    <img
                      ...
                      src={`http://localhost:7774/${service?.provider?.logo}`}
                      alt="Avatar"
                    />
                  </a>
